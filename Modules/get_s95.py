...
def calc_CLs(n_obs, nuTil_b_obs, b0_in, db_in, s0_in, nPseudo):
  ...
  ts,tb=[],[] #DL
  for i in range(NSIG):
    ts.append(qMu(n_pseu_sig[i], 0.0)) #DL
    #nuTil_b_pseu_sig[i] = 0.0 #DL
    if qMu(n_pseu_sig[i], nuTil_b_pseu_sig[i]) >= qMu_obs:
      positive_sig += 1
  for i in range(NBKG): 
    tb.append(qMu(n_pseu_bkg[i], 0.0)) #DL
    #nuTil_b_pseu_bkg[i] = 0.0 #DL
    if qMu(n_pseu_bkg[i], nuTil_b_pseu_bkg[i]) >= qMu_obs:
      positive_bkg += 1
  
  #DL CHECK
  # plt.hist(tb, bins=30, density=True, alpha = 0.4, label='b')   #DL
  # plt.hist(ts, bins=30, density=True, alpha = 0.4, label='s+b') #DL
  # plt.axvline(qMu_obs)
  # plt.text(qMu_obs*1.01,8,'t_obs={:.3f}'.format(qMu_obs),rotation=90)
  # plt.legend()
  # plt.savefig('CMtest.pdf')
  # exit()
...
def find_s95_exp(n_data, b0_in, sigma_b):  
